 ...
 * acknowledged</strong>.
 *
 * <p>A row is purge-eligible only when every registered consumer has acknowledged
 * it <em>and</em> its retention window has elapsed. Both conditions are in the one
 * statement below, and neither is optional: age alone would let an outage become
 * data loss, and acknowledgement alone would let the table grow without bound.
 *
