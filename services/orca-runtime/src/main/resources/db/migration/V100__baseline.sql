-- The first migration orca-runtime applies. It creates no tables — it labels the
-- schema, and by running at all it proves the service reached its own schema.
--
-- WHAT IT DOES
-- It attaches two labels to the `runtime` schema itself, using SQL Server's
...
-- months later.
--
-- The proof is that this file was applied at all. orca-runtime applied it on its
-- own startup, authenticating as the `orca_runtime` login, and Flyway recorded it
