 ...
 *
 * <p>{@code ModuleWallRule} closes another module's
 ...
 *
 * <p>The direction matters: <em>execution "raises a work item" → workitem</em>,
 * and <em>workitem "complete → advance" → execution</em>.
...
public interface WorkItemIntake {
	 ...
	 * together or not at all. An implementation that deferred this until after the
	 * commit would recreate the legacy 1.x system's two independent one-way calls,
	 * allowing either side to exist without the other.
	 *
	...
	/**
	 * Lane reset: fail every open item of this visit in the caller's transaction, so
	 * the visit and its work items fail <em>together</em>.
	 *
	 ...
	 *
	 * <p>Resolved at the moment the engine <em>arms</em> the boundary timer,
