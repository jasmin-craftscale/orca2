-- The first migration orca-fleet applies. It creates no tables — it labels the
-- schema, and by running at all it proves the service reached its own schema.
--
-- orca-fleet is an empty skeleton today: it owns a schema, boots, and does
...
--
-- WHAT IT DOES
-- It attaches two labels to the `fleet` schema itself, using SQL Server's
...
-- months later.
--
-- The proof is that this file was applied at all. orca-fleet applied it on its
-- own startup, authenticating as the `orca_fleet` login, and Flyway recorded it
