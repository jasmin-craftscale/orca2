-- The first migration orca-portal applies. It creates no tables — it labels the
-- schema, and by running at all it proves the service reached its own schema.
--
-- orca-portal is an empty skeleton today: it owns a schema, boots, and does
-- nothing else. It is only built out when work on the hosted cloud tier begins,
-- so this file being the whole of its schema is the current state of the service
-- rather than an omission.
--
-- WHAT IT DOES
-- It attaches two labels to the `portal` schema itself, using SQL Server's
...
-- months later.
--
-- The proof is that this file was applied at all. orca-portal applied it on its
-- own startup, authenticating as the `orca_portal` login, and Flyway recorded it
-- in a history table in that login's default schema. Wrong credentials or a wrong
...
IF NOT EXISTS (
	...
	WHERE class = 3 AND major_id = SCHEMA_ID(N'portal') AND name = N'orca_owner')
	EXEC sys.sp_addextendedproperty
		@name = N'orca_owner', @value = N'orca-portal',
		@level0type = N'SCHEMA', @level0name = N'portal';

IF NOT EXISTS (
	SELECT 1 FROM sys.extended_properties
	WHERE class = 3 AND major_id = SCHEMA_ID(N'portal') AND name = N'orca_chapter')
	EXEC sys.sp_addextendedproperty
		@name = N'orca_chapter', @value = N'§C4 — carriers and drivers',
		@level0type = N'SCHEMA', @level0name = N'portal';
